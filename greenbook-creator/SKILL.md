...
出图引擎：**gpt-5.5**（TokenRouter 路由），全篇统一 Prompt 体系 + 风格预设
...
- **生成工具**：`scripts/gen_image.sh`（TokenRouter + gpt-5.5），支持 `--style` 切换风格预设：
- `default`（浅色知识漫画）| `tech-dark`（深色科技风）| `tech-light`（浅色架构风）
- 新风格只需在 `scripts/styles/` 下新增 `.txt` 描述文件
- 默认 1024×1024，公众号配图需后处理压缩至 1MB 以下
...
- **出图流程**：`base_prompt` 只写一次 → 每页替换 `content_block` → 逐张调 `scripts/gen_image.sh --style default`
...
4. **生图** — 逐张调 `scripts/gen_image.sh --style default`，每张只改 `content_block`，保持其余 Prompt 完全一致
...
- 生图脚本：[scripts/gen_image.sh](scripts/gen_image.sh) — 支持风格预设，`--style default|tech-dark|tech-light`
- 风格预设：[scripts/styles/](scripts/styles/) — `default.txt`（知识漫画）/ `tech-dark.txt` / `tech-light.txt`
