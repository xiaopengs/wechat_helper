...
出图引擎：**gpt-image-2**（TokenRouter 路由），全篇统一 Prompt 体系
...
- **生成工具**：`gpt-image-2`（TokenRouter 路由，调用 `image-provider-constraint/scripts/gpt_image_2.sh`），默认 1024×1792（9:16）或 1792×1024（16:9）
...
- **出图流程**：`base_prompt` 只写一次 → 每页替换 `content_block` → 逐张调 `gpt-image-2`（TokenRouter）
...
4. **生图** — 逐张调 `gpt-image-2`（TokenRouter），每张只改 `content_block`，保持其余 Prompt 完全一致
