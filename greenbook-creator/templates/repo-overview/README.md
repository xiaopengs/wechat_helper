# repo-overview · 🗺️ 全景 · 开源项目全景（1080×1440 × 6P）

> **一句话**：Pillow 数据风，白底淡蓝渐变，6 页讲清开源项目元信息。用户说「用全景」「全景拆解」「6P 项目图」即触发本模板。

复用 OpenWiki 案例沉淀的小绿书模板：把任意开源项目，按 **封面 / 痛点 / 优势 / 双模式 / 竞品 / 上手** 六页讲清楚，配套一条 300 字"勾引使用"的总导语和六条单图配文。

## 🗺️ 全景 · 触发词 / 何时用

| 场景 | 该不该用 repo-overview |
|---|---|
| 你要 **6 页**讲清一个开源项目（封面/痛点/优势/双模式/竞品/上手） | ✅ **用全景** |
| 你想要 **Pillow 数据风**（白底淡蓝渐变，非卡通） | ✅ **用全景** |
| 内容以**项目元信息**为主（stars/模式/对比/安装），非架构图 | ✅ **用全景** |
| 你要 **8 张**固定骨架（封面/WHAT/ARCH/...） | ❌ 改用 🐒 猴子 / 🌿 小清新 |
| 你要拆 **架构图 / 流程图 / README+AGENTS** 长文档 | ❌ 改用 📐 蓝图 |

**触发关键词**（用户在 QQ/微信说这些词时自动匹配本模板）：
- **「全景」「全景风」「全景拆解」「6P 项目图」**（昵称触发）
- 「把 XX 开源项目做成 6 张图」「项目元信息图」
- 「Pillow 数据风」「开源项目全景」「OpenWiki 风格」
...
```bash
cp data/openwiki.yaml data/<新项目>.yaml      # 改 YAML 字段
python3 generate.py --data data/<新项目>.yaml --out output/<新项目>/
# 配文 → prompts/captions.md · 总导语 → prompts/topic-intro.md
```
